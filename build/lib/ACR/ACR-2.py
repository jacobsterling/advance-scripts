...
df['Number Of 3 Consecutive Payments'] = 0
for i, row in df.iterrows():
    df.at[i,'Number Of 3 Consecutive Payments'] = 1
    try:
        if df.at[i-1,'PAYNO'] == row['PAYNO']:
            if df.at[i-1,'Week'] == row['Week'] - 1:
                df.at[i,'Number Of 3 Consecutive Payments'] = df.at[i - 1,'Number Of 3 Consecutive Payments'] + 1
    except IndexError:
        pass
