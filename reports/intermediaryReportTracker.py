...
class intermediaryReportScript():
    def __init__(self):
        ...
        clients_io = dataPath / "clients io.csv"
        clients_axm = dataPath / "clients axm.csv"
        accounts = dataPath / "Accounts+Office.csv"
        groups_path = Path.home() / "OneDrive - advance.online/Documents/Data/Groups.xlsx"
        
        clients = pd.concat([pd.read_csv(clients_io,encoding = 'latin',
                                            usecols = ['Company Name                   ','OFFNO','EMAIL_DETAILS_INTER']),
                                pd.read_csv(clients_axm,encoding = 'latin',
                                            usecols = ['Company Name                   ','OFFNO','EMAIL_DETAILS_INTER'])])
        ...
        accounts = pd.read_csv(accounts, na_values = '-', skiprows=6)
        ...
        self.accounts = accounts.drop_duplicates(subset=['Office Number'], keep='first')
    ...
    def matchReport(self, offno):
        try:
            clientName = self.clients.loc[self.clients['OFFNO']==offno,'Client Name'].values[0]
            clientEmail = self.clients.loc[self.clients['OFFNO']==offno,'Email'].values[0]
        except IndexError:
            clientName = None
            clientEmail = None
            self.missingInfo += 2
        ...
        try:
            accountOwner = self.accounts.loc[self.accounts['Office Number']==offno,'Account Owner'].values[0]
            accountType = self.accounts.loc[self.accounts['Office Number']==offno,'Account Type'].values[0]
            crmEmail = self.accounts.loc[self.accounts['Office Number']==offno,'Send Intermediary Report to'].values[0]
        except IndexError:
            accountOwner = None
            accountType = None
            crmEmail = None
        ...
        for i, row in self.groups.iterrows():
    ...
    def queryReport(self, info, report):
        ...
        for k, report_row in iter_report.iterrows():
            ...
            if self.pd.to_datetime(report_row['End date of engagement']) == self.pd.to_datetime("19/04/2022" ,format="%d/%m/%Y"):
            ...
            if file.is_file():
                if file.suffix == ".csv" and self.report_prefix in file.name: 
                    ...
                    if not self.matchingQuery(info.columns[1].upper(), clientName):
                        if self.c == 1 and input(f"Miss Matching Client Name {info.columns[1].upper()}, type 'y' to change to {clientName}: ") == 'y':
                            self.changes += 1
...
missingData.to_csv("missing.csv", index = False)
